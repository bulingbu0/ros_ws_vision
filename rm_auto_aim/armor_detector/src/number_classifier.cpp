...
void NumberClassifier::extractNumbers(const cv::Mat & src, std::vector<Armor> & armors)
{
  ...
  for (auto & armor : armors) {
    ...
    // cv::Mat show_image;
    // cv::resize(number_image, show_image, cv::Size(), 20.0, 20.0);
    // cv::cvtColor(show_image, show_image, cv::COLOR_RGB2BGR);
    ...
    cv::namedWindow("number_image", cv::WINDOW_AUTOSIZE);
    // 显示图像
...
void NumberClassifier::classify(std::vector<Armor> & armors)
{
  for (auto & armor : armors) {
    ...
    // std::cout << "class_names_[label_id]: " << class_names_[label_id] << std::endl;
